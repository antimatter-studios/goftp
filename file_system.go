...
func (c *Client) Mkdir(path string) (string, error) {
	...
	if err != nil {
		...
		//
		// A reply without a name is a success we cannot read the name
		// out of, not a failure, so answer with the path that was asked
		// for rather than turning a created directory into an error.
		//
		// Getwd, the other caller of extractDirName, is different: there
		// the pathname *is* the answer, so it still fails.
		return path, nil
	}
